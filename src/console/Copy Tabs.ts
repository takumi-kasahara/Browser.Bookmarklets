...
(async () => {
  const gBrowser = (window as unknown as Window & {
    gBrowser?: {
  ...
  const urls = tabs.map(tab => tab.canonicalUrl).filter((u): u is string => Boolean(u));
  const anchors = tabs
    .filter((tab): tab is { label?: string; canonicalUrl: string } => Boolean(tab.canonicalUrl))
    .map(tab => createAnchorElement(tab.canonicalUrl, tab.label ?? tab.canonicalUrl));
  if (urls.length > 0)
