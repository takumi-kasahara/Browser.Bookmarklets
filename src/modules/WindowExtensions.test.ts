...
import { equiv, exists, from, is, open, select, tryFetch } from './WindowExtensions.js';
...
describe('select', () => {
  ...
  it('expands the range to the ancestor element', () => {
    document.body.innerHTML = '<div id="root"><span id="child">text</span></div>';
...
describe('from', () => {
  ...
  it('uses href from an object', () => {
    expect(from({ href: 'https://example.com/a' }).href).toBe('https://example.com/a');
  });

  it('uses src from an object', () => {
    expect(from({ src: 'https://example.com/a' }).href).toBe('https://example.com/a');
  });
...
describe('equiv', () => {
  it('matches identical origin, path, and query', () => {
    expect(equiv('https://example.com/a?x=1', 'https://example.com/a?x=1')).toBe(true);
  });
  ...
  it('detects differing query values', () => {
    expect(equiv('https://example.com/a?x=1', 'https://example.com/a?x=2')).toBe(false);
  });
...
describe('tryFetch', () => {
  ...
  it('returns exists:true on HEAD 200', async () => {
    ...
    vi.stubGlobal('fetch', vi.fn(async () => mockResponse(200, 'https://example.com/a')));
  ...
  it('returns exists:false on 404', async () => {
    ...
    vi.stubGlobal('fetch', vi.fn(async () => mockResponse(404, 'https://example.com/a')));
    const result = await tryFetch(new URL('https://example.com/a'));
